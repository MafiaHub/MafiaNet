...
namespace
{
	...
	// TryDisconnectCase outcomes. Non-negative values are concrete ErrorCodeToString
	// codes; negative values are transient signals the caller may retry.
...
int DisconnectReasonTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses)
{
	...
	// --- Case 1: graceful disconnect WITH a reason payload ---
	{
		...
		int err = RunDisconnectCaseWithRetries(server, client, serverPort, &reason, true,
			...
			isVerbose, "Case 1 OK: reason delivered.");
		if (err != 0)
			return err;
	}
	...
	// --- Case 2: graceful disconnect WITHOUT a reason (nullptr default) ---
	{
		...
		int err = RunDisconnectCaseWithRetries(server, client, serverPort, nullptr, false,
			...
			isVerbose, "Case 2 OK: reasonless notification is a bare 1-byte message.");
		if (err != 0)
			return err;
	}
	...
	// --- Case 3: graceful disconnect with an EMPTY (non-null) reason BitStream ---
	{
		...
		int err = RunDisconnectCaseWithRetries(server, client, serverPort, &emptyReason, false,
			...
			isVerbose, "Case 3 OK: empty reason BitStream yields a bare 1-byte message.");
		if (err != 0)
			return err;
	}
