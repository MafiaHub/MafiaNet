...
{
	// This stress test destroys and recreates client peers mid-flight (see
	// DestroyInstance/GetInstance below) while their connections and network
	// threads are still live. It used to be quarantined under CI because RakPeer
	// teardown could abandon its recv polling thread and free state under it
	// (https://github.com/MafiaHub/MafiaNet/issues/7); teardown now joins both
	// internal threads, so the test runs everywhere again.
