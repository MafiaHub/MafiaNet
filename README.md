...
On Linux, MafiaNet coalesces multiple datagrams into a single `recvmmsg`/`sendmmsg`
...
- **Fixed**: a locally-refused MTU burned its rung's whole attempt budget (the check compared `Send()`'s return value against `10040`, which `sendto` never returns); a `sendto` blocking over 100 ms abandoned the connection attempt outright; and `Connect()` divided by zero for any `sendConnectionAttemptCount` below the number of MTU rungs
- **Non-breaking**: `RAKNET_PROTOCOL_VERSION` stays at 7 and no message ids move. Peers converge on the smaller of their two caps, so a server rebuilt on 0.16.0 caps every connection — including clients still built against 0.15.0 — with no client update required
